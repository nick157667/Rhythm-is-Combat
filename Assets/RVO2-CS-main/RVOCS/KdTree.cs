...
namespace RVO
{
    ...
    internal class KdTree
    {
        ...
        {
            // 若 agents_ 是第一次建構，或 agents 數量改變
        ...
        private void buildAgentTreeRecursive(int begin, int end, int node)
        {
            ...
            if (end - begin > MAX_LEAF_SIZE)
            {
                ...
                // 將agents_重新排序使 左側小於 splitValue, 右側大於等於 splitValue
                while (left < right)
                {
        ...
        private ObstacleTreeNode buildObstacleTreeRecursive(IList<Obstacle> obstacles)
        {
            ...
            // 在障礙邊（Obstacle）中，找出一條最適合分割障礙樹（ObstacleTree）的邊，左/右側的數量越平均越好。
            ...
            // 根據剛剛選出的最佳分割邊，將所有障礙分為左右兩邊，並建立當前節點（ObstacleTreeNode）及其子樹
            {
                /* Build split node. */
                ...
                // 這條邊（obstacleI1 → obstacleI2）剛剛在上個步驟中選為最佳分割線。
                Obstacle obstacleI1 = obstacles[i];
                Obstacle obstacleI2 = obstacleI1.next_;
                ...
                for (int j = 0; j < obstacles.Count; ++j)
                {
                    ...
                    // 跨越分割線（Split Line）時，將它切成兩段並分別放入左右子節點。
                    else
                    {
                        /* Split obstacle j. */
         ...
         * 在半徑 radius 的範圍內，點 q1 和 q2 之間是否存在「沒有被障礙物擋住」的直線可視線（Line of Sight）
         * </summary>
         *
         * <returns>True if q1 and q2 are mutually visible within the radius;
         * false otherwise.</returns>
